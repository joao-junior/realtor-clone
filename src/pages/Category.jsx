...
function Category() {
  ...
  async function onFetchMoreListings() {
    try {
      const listingRef = collection(db, "listings")
      const q = query(listingRef, where("type", "==", params.categoryName), orderBy("timeStamp", "desc"), startAfter(lastFetchedList), limit(4))
      const querySnap = await getDocs(q)
      const lastVisible = querySnap.docs[querySnap.docs.length - 1]
      setLastFetchedList(lastVisible)
      const listings = []
  ...
  return (
    <div className='max-w-6xl mx-auto px-3'>
      <h1 className='text-center text-3xl mt-6 font-bold mb-6'>{params.categoryName === "rent" ? "Places for rent" : "Places for sale"}</h1>
      {loading ? (
        <Spinner />
